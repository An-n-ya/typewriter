...
lazy_static! {
    static ref 並擊轉拼音: Box<[拼寫運算::<'static>]> = Box::new([
        ...
        // 特別地，⟨me⟩ 對應常用字「麼·么」
        變換!("^([mdtnlgkh])([1234])?$", "${1}e${2}"),
        
        // 只保存最後一個音調         
        變換!("^([a-z]+)([1234]+)([1234])$", "${1}${3}"),

        // 檢查拼音音節，通過檢查則追加隔音符號
        變換!("^([bpm])([iu]|a|i?e|o|[ae]i|i?ao|[oi]u|i?an|[ie]n|[ei]ng|ang|ong)([1234])?$", "$1$2$3'"),
        變換!("^([fw])(u|a|o|[ae]i|ao|ou|an|en|eng|ang|ong)([1234])?$", "$1$2$3'"),
        變換!("^([dt])([iu]|i?a|i?e|uo|[aeu]i|i?ao|[oi]u|[iu]?an|[ue]n|[ei]ng|ang|ong)([1234])?$", "$1$2$3'"),
        變換!("^([nl])([iuv]|i?a|[iv]?e|u?o|[aeu]i|i?ao|[oi]u|[iu]?an|[iue]n|[ei]ng|i?ang|ong)([1234])?$", "$1$2$3'"),
        變換!("^([gkh])(u|u?a|e|uo|u?ai|[ue]i|ao|ou|u?an|[ue]n|eng|u?ang|ong)([1234])?$", "$1$2$3'"),
        變換!("^([zcs]h?|r)([iu]|u?a|e|uo|u?ai|[ue]i|ao|ou|u?an|[ue]n|eng|u?ang|ong)([1234])?$", "$1$2$3'"),
        變換!("^([jqxy])([iu]|i?a|[iu]?e|o|i?ao|[oi]u|[iu]?an|[iu]n|ing|i?ang|i?ong)([1234])?$", "$1$2$3'"),
        變換!("^([aeo]|[ae]i|ao|ou|[ae]ng?|er)([1234])?$", "$1$2'"),
        // 消除不構成合法音節的並擊組合
        消除!("^[A-Za-z]+$"),
        轉寫!("v", "ü"),
        // 顯示單個音節不需要加隔音符號
...
const 宮保語音盤面: 盤面定義<'static> = 盤面![
    [ _ _ _ _ _ _ _ _ _ _ _ _ _ _ ],
    [ "4" {中:C, 上:_, 下:_, 左:_, 右:"ĉ"} {中:L, 上:_, 下:_, 左:"ĉ", 右:"n"} {中:D, 上:_, 下:_, 左:"n", 右:_} "2" "4" U {中:R, 上:"er", 下:_, 左:"-i", 右:"-u"} O "2" _ ],
    [ "3" {中:S, 上:_, 下:_, 左:_, 右:"ŝ"} {中:H, 上:"x", 下:_, 左:"ŝ", 右:"r"} {中:G, 上:_, 下:_, 左:"r", 右:"j"} "1" "3" I {中:N, 上:_, 下:_, 左:"-n", 右:"-ŋ"} {中:E, 上:_, 下:_, 左:"-ŋ", 右:_} "1" _ _ ],
    [ 空 {中:Z, 上:_, 下:_, 左:_, 右:"ẑ"} {中:F, 上:_, 下:_, 左:"ẑ", 右:"m"} {中:B, 上:_, 下:_, 左:"m", 右:_} 空 空 Ü _ _ _ ],
    [ A _ A ]
];

const 宮保語音鍵盤佈局: 鍵盤佈局 = 鍵盤佈局 {
    盤面: &[基本盤面, 上檔盤面, 大寫字母盤面, 宮保語音盤面],
    默認盤面: 盤面選擇碼(4),
    首選配列: 配列::正交直列,
};

pub fn 宮保語音輸入方案() -> 輸入方案定義<'static> {
    輸入方案定義 {
        名稱: "宮保語音",
        佈局: &宮保語音鍵盤佈局,
        指法: 觸鍵方式::並擊,
